...
export function TestcaseForm({ problemId }: TestcaseFormProps) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    formData.append("score", (event.currentTarget.querySelector('[name="score"]') as HTMLInputElement)?.value || "10");
    formData.append("isHidden", (event.currentTarget.querySelector('[name="isHidden"]') as HTMLInputElement)?.checked ? "true" : "false");
  ...
  return (
    <form ref={formRef} onSubmit={onSubmit} className="space-y-4">
      ...
      <Button 
        type="submit" 
        className="w-full" 
        disabled={isSubmitting || !inputFile || !outputFile}
      >
        {isSubmitting ? (
          <>
