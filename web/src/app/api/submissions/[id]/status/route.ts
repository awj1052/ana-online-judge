...
import { db } from "@/db";
import { submissions, submissionResults } from "@/db/schema";
    ...
    try {
      ...
      if (resultJson) {
        ...
        await db
          .update(submissions)
          .set({
            verdict: result.verdict as "pending" | "judging" | "accepted" | "wrong_answer" | "time_limit_exceeded" | "memory_limit_exceeded" | "runtime_error" | "compile_error" | "system_error",
        ...
        if (result.testcase_results && result.testcase_results.length > 0) {
          ...
          await db.insert(submissionResults).values(
            result.testcase_results.map((tc) => ({
              ...
              verdict: tc.verdict as "pending" | "judging" | "accepted" | "wrong_answer" | "time_limit_exceeded" | "memory_limit_exceeded" | "runtime_error" | "compile_error" | "system_error",
