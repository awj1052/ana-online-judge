...
import { Clock, HardDrive, Code2, User, Calendar } from "lucide-react";
...
export default async function SubmissionDetailPage({ params }: Props) {
  ...
  const verdictInfo = VERDICT_LABELS[submission.verdict] || { label: submission.verdict, color: "bg-gray-500" };
            ...
            <div className="flex items-start justify-between">
              <div>
                <CardTitle className="text-2xl flex items-center gap-2">
                  제출 #{submission.id}
                  <SubmissionStatus submissionId={submission.id} initialVerdict={submission.verdict} />
                </CardTitle>
                <Link
                ...
                <Table>
                  ...
                  <TableBody>
                    {submission.testcaseResults.map((result, index) => {
                      const tcVerdictInfo = VERDICT_LABELS[result.verdict] || { label: result.verdict, color: "bg-gray-500" };
                      return (
                        <TableRow key={result.id}>
                          <TableCell className="font-mono">{index + 1}</TableCell>
                          <TableCell>
                            <Badge className={`${tcVerdictInfo.color} hover:${tcVerdictInfo.color}`}>
                              {tcVerdictInfo.label}
                            </Badge>
                          </TableCell>
